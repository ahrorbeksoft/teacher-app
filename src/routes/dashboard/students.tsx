...
} from "@/components/reusables/data-table/data-table";
...
function RouteComponent() {
  ...
  const options: DataTableOptions<Student, unknown> = {
          ...
          return (
            <>
              {birthday ? (
                ...
                <Badge className="font-semibold text-white text-xs bg-gray-500">
                  {t("n-a")}
                </Badge>
              )}
            </>
    ...
    bulkActions: [
      {
        ...
        confirm: {
          ...
          body: t("really-delete-plural", {
            name: t("students").toLowerCase(),
          }),
        ...
        onClick: async (rows) => {
          try {
            await db.transact(
              rows.map((r) => db.tx.students[r.original.id].delete()),
            );
            toast.success(
              t("toast.delete-successfull-singular", { name: t("students") }),
            );
          } catch (err) {
    ...
    Buttons: (
      <>
        <Button onClick={() => setStudentSheetOpen(true)}>
          {t("add-student")}
        </Button>
      </>
    ),
  ...
  return (
    <>
      ...
      <DataTable
        data={data?.students || []}
        scrollableClassName="h-[calc(100vh-170px)]"
        options={options}
      />
    </>
  );

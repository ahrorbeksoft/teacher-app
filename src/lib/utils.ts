	...
	switch (lesson_day) {
		...
		case "custom":
			if (!customDays || customDays.length === 0) {
				throw new Error("Custom lesson days must be provided when lesson_day is 'custom'");
			}
...
export function getLessonDatesGroupedByMonth(
	...
	while (!isAfter(currentMonthStart, lastMonthEnd)) {
		...
		const rangeStart = isBefore(startDate, currentMonthStart) ? currentMonthStart : startDate;
		const rangeEnd = isBefore(endDate, currentMonthEnd) ? endDate : currentMonthEnd;

		const allDays = eachDayOfInterval({ start: rangeStart, end: rangeEnd });

		const lessons = allDays.filter((date) => validWeekdays.includes(date.getDay()));
...
export function getTotalMonthlyLessons(
	...
	switch (lesson_day) {
		...
		case "custom":
			if (!customDays || customDays.length === 0) {
				throw new Error("Custom weekdays must be provided for 'custom' lesson day.");
			}
...
export function getMonths(startDate: Date | string | number, finishDate: Date | string | number): Date[] {
	const dates: Date[] = [];
